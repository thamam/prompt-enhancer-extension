...
// Platform-agnostic core enhancement logic
...
class PromptEnhancer {
  ...
  async enhanceWithLocalLLM(prompt) {
    ...
    if (!settings.enabled) {
      throw new Error('LLM is not enabled. Please configure it in the extension settings.');
    }

    // Validate settings based on provider
    if (settings.provider === 'local') {
      if (!settings.endpoint || !settings.model) {
        throw new Error('Local LLM is not configured. Please set endpoint and model in the extension settings.');
      }
    } else {
      // Remote provider
      if (!settings.apiKey || !settings.model) {
        throw new Error('Remote LLM is not configured. Please set API key and model in the extension settings.');
      }
    }
