...
// Load the enhancer and security scanner (using window.* to access globally defined classes)
const enhancer = new window.PromptEnhancer();
...
function enhanceAndReplace(text, mode) {
  ...
  if (scanResult.hasMediumIssues || scanResult.hasLowIssues) {
    showNotification(`⚠️ Security Score: ${scanResult.score}/100 - ${scanResult.totalIssues} issue(s) detected`, 'warning');
  }
...
async function performEnhancement(text, mode, isRedacted = false) {
  try {
    ...
    showNotification(`✓ Prompt enhanced with ${modeName}${securityNote}`, 'success');
...
function replaceSelectedText(newText) {
  ...
  if (!currentRange) {
    showNotification('Error: Text selection was lost. Please select the text again.', 'error');
